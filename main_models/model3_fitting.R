...
init_func <- function(...) {
  list(beta_par = 3*10^-11,
       ...
       theta_mult = array(rep(1.0, times = 5)),
       phi =  2,
       lambda = 5 )}
...
param_to_save = c("beta_par", "c_par", "gamma_par", "omega_par", "theta", "gamma_mod", "log_lik", "sumloglike")

rstan_options(disable_march_warning = TRUE)
test = stan("model3.stan",
            data = dfst, 
            chains = 4,
            iter = 3000, 
            warmup = 1500,
...
# save test
saveRDS(test, "model3_delta_op.rds")
...
param_to_save = c("beta_par", "c_par", "gamma_par", "omega_par", 
                  "theta", "gamma_mod", "log_lik", "sumloglike")
...
# save test
saveRDS(test, "model3_omicron_op.rds")
