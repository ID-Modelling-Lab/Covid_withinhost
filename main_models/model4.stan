...
parameters {
  ...
  real<lower = 0> phi;
  real<lower = 0> lambda;
  real<lower = 0> theta_mult_g[5]; // total 6 values for theta, theta[1] = 1.0
  real<lower = 0> theta_mult_o[5]; // total 6 values for theta, theta[1] = 1.0
}
...
transformed parameters {
  ...
  real params[4];
  real y0[4];
  real<lower = 0> theta_g[6];
  real<lower = 0> theta_o[6];
  ...
  
  {
    ...
    theta_g[which_fixed] = fixed_value;
    theta_o[which_fixed] = fixed_value;
    for (i in 2:6) {
      theta_g[i] = theta_g[which_fixed] * theta_mult_g[i-1];
      theta_o[i] = theta_o[which_fixed] * theta_mult_o[i-1];
    }
    //
    ...
    for (ag in 1:AGtsv) {
      ...
      gamma_mod[ag] = theta_g[pos_AGofAGtsv[ag]] * gamma_par[pos_VGtsvofAGtsv[ag]];
      omega_mod[ag] = theta_o[pos_AGofAGtsv[ag]] * omega_par[pos_VGtsvofAGtsv[ag]];
