...
param_to_save = c("beta_par", "c_par", "gamma_par", "omega_par", "log_lik", "sumloglike")

rstan_options(disable_march_warning = TRUE)
test = stan("model1.stan",
            data = dfst, 
            chains = 4,
            iter = 3000, 
            warmup = 1500,
...
# save test
saveRDS(test, "model1_delta_op.rds")
...
# save test
saveRDS(test, "model1_omicron_op.rds")
